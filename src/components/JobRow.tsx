	...
	return (
		<div className="job-row">
			...
			>
				...
				<span className="job-location">{job.location ? `📍 ${job.location}` : ""}</span>
			...
			{open && (
				<div className="job-row-panel">
					...
					{(Object.entries(STATUS_CONFIG) as [JobStatus, typeof s][]).map(
						([key, cfg]) => (
							<button
								...
								style={{
									borderColor: job.status === key ? cfg.color : "transparent",
									background: cfg.bg,
									color: cfg.color,
								}}
								disabled={busy}
					...
					{error && (
						<div className="auth-error" style={{ width: "100%", marginTop: "8px" }}>
							{error}
						</div>
					)}
				</div>
